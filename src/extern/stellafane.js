...
export default {
  ...
  //-----Utility Funtions------------------------------------------------------------
  INT: function ( n ) { return Math.floor(n); },  // Emulates BASIC's INT Funtion
  POW2: function POW2( n ) { return Math.pow(n,2); },  // Square a number
  POW3: function( n ) { return Math.pow(n,3); },  // Cube a number
  POW4: function ( n ) { return Math.pow(n,4); },  // Number to the 4th power
  COS: function ( deg ) {             // Cosine function with degrees as input
  ...
  // Meeus Astronmical Algorithms Chapter 27
  calcInitial: function ( k, year ) { // Valid for years 1000 to 3000
    var JDE0=0, Y=(year-2000)/1000;
    switch( k ) {
      case 0: JDE0 = 2451623.80984 + 365242.37404*Y + 0.05169*EquinoxCalc.POW2(Y) - 0.00411*EquinoxCalc.POW3(Y) - 0.00057*EquinoxCalc.POW4(Y); break;
      case 1: JDE0 = 2451716.56767 + 365241.62603*Y + 0.00325*EquinoxCalc.POW2(Y) + 0.00888*EquinoxCalc.POW3(Y) - 0.00030*EquinoxCalc.POW4(Y); break;
      case 2: JDE0 = 2451810.21715 + 365242.01767*Y - 0.11575*EquinoxCalc.POW2(Y) + 0.00337*EquinoxCalc.POW3(Y) + 0.00078*EquinoxCalc.POW4(Y); break;
      case 3: JDE0 = 2451900.05952 + 365242.74049*Y - 0.06223*EquinoxCalc.POW2(Y) - 0.00823*EquinoxCalc.POW3(Y) + 0.00032*EquinoxCalc.POW4(Y); break;
    }
    return JDE0;
  }, // End calcInitial
